...
def value_chain_image(filename):
    return send_from_directory('static/value_chain', filename)
...
def chat():
    try:
        ...
        if "value chain" in user_message:
            for keyword, filename in value_chain_images.items():
                if keyword in user_message:
                    ...
                    return jsonify({
                        'response': f"Here is the {keyword.title()} value chain.",
                        'image': image_url
                    }), 200
        ...
        # Generate a response using the conversation chain, passing context
        response = conversation.predict(human_input=user_message, context=context)
...
# Remove the development server code for deployment
